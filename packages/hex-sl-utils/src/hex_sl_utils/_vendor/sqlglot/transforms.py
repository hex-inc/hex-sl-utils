...
from hex_sl_utils._vendor.sqlglot.errors import UnsupportedError
from hex_sl_utils._vendor.sqlglot.helper import find_new_name, name_sequence


if t.TYPE_CHECKING:
    from hex_sl_utils._vendor.sqlglot._typing import E
    from hex_sl_utils._vendor.sqlglot.generator import Generator
...
def unqualify_unnest(expression: exp.Expression) -> exp.Expression:
    ...
    from hex_sl_utils._vendor.sqlglot.optimizer.scope import find_all_in_scope
...
def explode_projection_to_unnest(
    ...
    def _explode_projection_to_unnest(expression: exp.Expression) -> exp.Expression:
        if isinstance(expression, exp.Select):
            from hex_sl_utils._vendor.sqlglot.optimizer.scope import Scope
...
def ensure_bools(expression: exp.Expression) -> exp.Expression:
    ...
    from hex_sl_utils._vendor.sqlglot.optimizer.canonicalize import ensure_bools
...
def eliminate_join_marks(expression: exp.Expression) -> exp.Expression:
    ...
    from hex_sl_utils._vendor.sqlglot.optimizer.scope import traverse_scope
    from hex_sl_utils._vendor.sqlglot.optimizer.normalize import normalize, normalized
